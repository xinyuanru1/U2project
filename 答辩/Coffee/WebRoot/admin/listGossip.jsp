...
<script type="text/javascript">
	function condelete(){
		...
		if(isdelete){
			...
			$("#delId").attr("href","../GossipServlet?flag=getAll");
		}
	}
</script>
</head>
...
<ul>
 ...
 <div id="dcMain">
   <!-- 当前位置 -->   <div class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
        <h3><a href="../admin/addGossip.jsp" class="actionBtn add">添加留言</a>留言列表</h3>
        <form action="../GossipServlet?flag=getSome" method="post">
        	选择查询内容：<input type="text" name="text"/><input type="submit" value="确认"/>
        </form>
    <div class="filter">
    </div>
        <div id="list">
    <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
      <tr>
        ...
        <th width="40" align="center" >主题</th>
      ...
      <c:forEach items="${gossips}" var="gossip">
      		<tr>
		        ...
		        <td align="center"><a href="../GossipServlet?flag=getForUpd&gossipNo=${gossip.gossipNo}">编辑</a>
		         | <a id="delId" name="asdf" href="GossipServlet?flag=delete&gossipNo=${gossip.gossipNo}" onclick="condelete()">删除</a>
		         | <a href="../GossipServlet?flag=toReply&gossipNo=${gossip.gossipNo}">回复</a>
		          </td>
		     </tr>
      </c:forEach>
	</table>
    ...
    <c:if test="${pageIndex>1}">
    	<a href="../GossipServlet?flag=getAll&pageIndex=1">第一页</a> <a href="../GossipServlet?flag=getAll&pageIndex=${pageIndex-1}">上一页</a> 
    </c:if>
    <c:if test="${pageIndex<pageCount}">
    	<a href="../GossipServlet?flag=getAll&pageIndex=${pageIndex+1}">下一页</a> <a href="../GossipServlet?flag=getAll&pageIndex=${pageCount}">最末页</a>
    </c:if>
