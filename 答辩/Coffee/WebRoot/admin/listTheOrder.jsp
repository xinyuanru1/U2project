...
<script type="text/javascript">
	function condelete(){
		...
		if(isdelete){
			$("#delId").attr("href","../OrderServlet?flag=delete&orderNo="+asdf);
		}else{
			$("#delId").attr("href","../OrderServlet?flag=getTheOrder");
		}
	}
</script>
</head>
...
<ul>
 ...
 <div id="dcMain">
   <!-- 当前位置 -->   <div class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
        <h3><a href="../admin/addOrder.jsp" class="actionBtn add">添加订单</a>订单列表</h3>
        <form action="../OrderServlet?flag=getTheOrder" method="post">
        	查询名字：<input type="text" name="name" value="${name}"/><input type="submit" value="确认"/>
        </form>
    <div class="filter">
    </div>
        <div id="list">
    <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
      ...
      <c:forEach items="${orders}" var="order">
      		<tr>
		        ...
		        <td><a href="../OrderServlet?flag=getOrderForUpd&orderNo=${order.orderNo}">编辑</a> | <a id="delId" href="OrderServlet?flag=delete&orderNo=${order.orderNo}" onclick="condelete()">删除</a></td>
		     </tr>
      </c:forEach>
	</table>
    </div>
   ...
   <div class="pager">共 ${totalPage}  页，当前第 ${pageIndex }页 | 
    <c:if test="${pageIndex>1}">
    	<a href="../OrderServlet?flag=getTheOrder&name=${name}">第一页</a>
    	<a href="../OrderServlet?flag=getTheOrder&pageIndex=${pageIndex-1}&name=${name}">上一页</a> 
    </c:if>
   	<c:if test="${pageIndex<totalPage}">
   		<a href="../OrderServlet?flag=getTheOrder&pageIndex=${pageIndex+1}&name=${name}">下一页 </a> 
   		<a href="../OrderServlet?flag=getTheOrder&pageIndex=${totalPage}&name=${name}">最末页</a>
   	</c:if>
   </div>     
  </div>
 </div>
