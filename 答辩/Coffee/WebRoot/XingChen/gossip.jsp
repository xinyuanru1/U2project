...
<div id="wrapper">
	<div id="content">
		...
		<header class="header--image" style="background-image: url(images/newsroom-beans-july-20150721130557-r-h280-q100-m111111.jpg);">
		<div class="container">
			<div class="row">
			...
			<div class="row">
				<div class="col-md-7">
					<div class="news__date">
						时间：${gossip.gossipTime} 
					</div>
					<!--  --><h5>用户：***</h5>
					主题：${gossip.gossipTitle}<br/>
${gossip.gossipContent}

<p></p>
<c:choose>
	<c:when test="${user.statu==0}">
		<a href="../GossipServlet?flag=addGood&userId=${user.id}&gossipNo=${gossip.gossipNo}">赞：</a>${gossip.goods}		
		<a href="../GossipServlet?flag=addBad&userId=${user.id}&gossipNo=${gossip.gossipNo}">踩：</a>${gossip.bads}
	</c:when>
	<c:otherwise>
		已评论&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		赞：${gossip.goods}		
...
<!-- <div align="center"><a href="#">顶&nbsp &nbsp0次</a>&nbsp &nbsp<a href="#">踩 &nbsp &nbsp0次</a></div><p></p> -->
<div>星辰回复：${gossip.reply}<br/>回复时间：${gossip.replyTime}</div>				</div>
			</div>
			</c:forEach>
			<a href="addGossip.jsp" align="right" style="color:red">我要留言</a>
			<nav class="text-center">
			<div class="pager">共${pageCount} 页，当前第${pageIndex} 页 
    <c:if test="${pageIndex>1}">
    	<a href="../GossipServlet?flag=gossip">第一页</a>
    	<a href="../GossipServlet?flag=gossip&pageIndex=${pageIndex-1}">上一页</a> 
    </c:if>
   	<c:if test="${pageIndex<pageCount}">
   		<a href="../GossipServlet?flag=gossip&pageIndex=${pageIndex+1}">下一页 </a> 
   		<a href="../GossipServlet?flag=gossip&pageIndex=${pageCount}">最末页</a>
   	</c:if>
